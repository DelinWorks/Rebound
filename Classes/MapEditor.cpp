...
void MapEditor::editUpdate_place(f32 _x, f32 _y, f32 _width, f32 _height) {
    ...
    undoCmd.setAction(Editor::UNDOREDO_TILEMAP);
...
void MapEditor::editorUndo()
{
    if (_undo.size() > 0) {
        ...
        if (state.isStateExplicit)
        {
            _redo.push(state);
            _undo.pop();
            if (_undo.size() > 0)
                _undo.top().applyExplicitState();
        }
        else {
            _redo.push(state);
...
GameUtils::Editor::UndoRedoState* MapEditor::editorTopUndoStateOrNull()
{
...
CUI::Container* MapEditor::createFledgedHSVPanel() {
    ...
    hsv->_onClickCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    ...
    hextf->_onFocusCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    ...
    tfr->_onFocusCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    tfr->setStyleDotted();
    ...
    tfg->_onFocusCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    tfg->setStyleDotted();
    ...
    tfb->_onFocusCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    tfb->setStyleDotted();
    ...
    tfa->_onFocusCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    tfa->setStyleDotted();
    ...
    rightPageB->_callback = [=](Button* target) {
        Functions::fillContainerColorGrid(manager, channelCont, rowcol.u, rowcol.v, ++channelCont->_userData.index, onColorSelect);
    ...
    channelCont->_onContainerTabSelected = [=](Container* s) {
        Functions::fillContainerColorGrid(manager, s, rowcol.u, rowcol.v, channelCont->_userData.index, onColorSelect);
    ...
    hsv->_callback = [=](const HSV& hsv, HSVWheel* target) {
        ...
        auto newCol = manager->getColor(target->currentChannel);
        newCol.color = col;
        auto state = editorTopUndoStateOrNull();
        if (state) state->affectedColors.setColor(newCol);
    };

    slr->_onClickCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    ...
    slg->_onClickCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    ...
    slb->_onClickCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
    ...
    sla->_onClickCallback = CALL0(MapEditor::colorPaletteUndoRedoPush);
